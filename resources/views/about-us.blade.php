@extends('layouts.forntend')
  @section('title')
  PickiMarket | Online Shopping for Computers, Laptops, Phones, Accessories &amp; More
  @endsection
  @section('fcontent')   
        <main class="main">
            <nav class="breadcrumb-nav">
                <div class="container">
                    <ul class="breadcrumb">
                        <li><a href="{{url('/')}}"><i class="d-icon-home"></i></a></li>
                        <li>About Us</li>
                    </ul>
                </div>
            </nav>
            <div class="page-header pl-4 pr-4" style="background-image: url({{asset('images/about-us.jpg')}})">
                {{-- <h3 class="page-subtitle font-weight-bold">About Us</h3> --}}
                <h1 class="page-title font-weight-bold lh-1 text-white text-capitalize">About Us</h1>
                {{-- <p class="page-desc text-white mb-0">Lorem quis bibendum auctor, nisi elit consequat ipsum,<br> nec
                    sagittis sem nibh id elit.</p> --}}
            </div>
            <div class="page-content mt-10 pt-10">
                <div class="container">
                    <div style="border-bottom: 2px solid orange; text-align:center">
                        <ul class="list-style-none">
                          <span style="font-weight: 700;"><a href="#about" class="text-decoration-none ml-3">Picki Market</a></span>
                          <span style="font-weight: 700;"><a href="#vision" class="text-decoration-none ml-3">Our Vision</a></span>
                          <span style="font-weight: 700;"><a href="#history" class="text-decoration-none ml-3">Our History</a></span>
                        </ul>
                      </div>
                </div>
                <section class="about-section pb-10 appear-animate" id="about">
                    <div class="container">
                        <h2 class="text-center">About Us</h2>
                       {!!$abtpg ? $abtpg->content : ''!!}
                    </div>
                </section>
                <section class="about-section pb-10 appear-animate" id="vision">
                    <div class="container">
                         <h2 class="text-center">Our Vision</h2>
                       {!!$vis ? $vis->content : ''!!}
                    </div>
                </section>
                <section class="about-section pb-10 appear-animate" id="history">
                    <div class="container">
                         <h2 class="text-center">Our History</h2>
                       {!!$his ? $his->content : ''!!}
                    </div>
                </section>
                <!-- End About Section-->

            </div>
        </main>
       
    @endsection